...
COMMANDS = frozenset({"/reset", "/clear", "/cancel", "/model", "/help", "/status"})
...
class ChatActor:
    ...
    async def _handle_reset(self):
        """Handle /reset. Kills loop, clears ALL state, stops actor."""
        ...
        self._stopped = True  # _run() checks this after _handle() returns
        # Sentinel ensures _run() unblocks if re-entered or awaiting queue.get()
        with contextlib.suppress(asyncio.QueueFull):
            self.queue.put_nowait(None)

    async def _handle_clear(self):
        """Handle /clear. New conversation but preserve settings (model)."""
        await self._kill_loop()
        self.loop_id = None
        self.session_id = ""
        self._save_state()
        self._send_text("Conversation cleared. Settings preserved.")

    def _send_help(self):
        self._send_text(
            "Commands:\n"
            "/cancel - Cancel current operation\n"
            "/clear - New conversation, keep settings\n"
...
async def _dispatch(event: IncomingEvent):
    ...
    if event.kind == "command":
        parsed = _parse_command(event.text)
        if parsed and parsed[0] in ("/reset", "/clear", "/cancel") and actor.loop_id:
