...
In any multi-step system, the order of operations is load-bearing. The wrong ordering produces spurious terms that must be cancelled by hand; the right one eliminates entire classes before expansion begins. (A [numerical check](canon://skills/numerical_evidence) must replicate this ordering exactly.)
...
Invoke when asked to derive a result from stated assumptions. Load the [result statement](canon://skills/result_foundry) and model assumptions first; do not expand the working context with prior derivations or manuscript text. If a closure condition is not identifiable at Step 3, flag it as an open gap and do not proceed to Step 5.
