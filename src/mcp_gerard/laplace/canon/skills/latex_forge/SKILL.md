...
The Forge Compiler is the auxiliary `scripts/compile_pdf.py` in this skill (resolve its directory via `laplace_skill("latex_forge").backing_path`). Run it with Python through the shell - on Windows, wrap it in the [`shell_mechanic`](canon://skills/shell_mechanic) safe executor to avoid UTF-16 and pathing friction:
