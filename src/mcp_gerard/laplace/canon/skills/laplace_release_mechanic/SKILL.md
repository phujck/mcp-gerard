...
Invoke the [`hostile_redteam_swarm`](canon://skills/hostile_redteam_swarm) skill. Have the three reviewers (Formalist, Architect, Pedant) aggressively assault the current draft. You must computationally repair any "naked claims" flagged by deriving the missing math.
...
## Phase 3: Structural Compilation ([Compilation and Release](canon://operations/compilation_and_release.md))
Pass the `.tex` files through the [`latex_forge`](canon://skills/latex_forge) compiler. Use the Forge Compiler (`compile_orchestra.py` or equivalent). Verify there are zero `xcolor` or `tikz` geometry warnings (consulting [`tikz_mechanic`](canon://skills/tikz_mechanic) if needed).
