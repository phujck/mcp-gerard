...
Each project gets its own Git repository under ~/.mcp-gerard/conversations/.
"""
...
def get_global_storage_dir() -> Path:
    ...
    base = Path(os.environ.get("MCP_GERARD_MEMORY_DIR", "~/.mcp-gerard"))
...
def _ensure_git_repo(project_dir: Path) -> None:
    ...
    # Set local identity
    _git(project_dir, "config", "user.name", "mcp-gerard")
    _git(project_dir, "config", "user.email", "mcp-gerard@local")
